...
class CustomPhoneme(Language):
    def __init__(self, rating, length_of_phoneme, size_of_vocab):
        super().__init__(size_of_vocab)
        self.rating = rating
        self.length_of_phoneme = length_of_phoneme
    ...
    def create_custom_phonemes(self):
        ...
        for letter in range(len(custom_phoneme), self.length_of_phoneme):
            random.shuffle(soft)
            try:
                custom_phoneme.append(self.SOFT.pop())
            except IndexError:
                continue
...
example = Language(50)
example_subclass = CustomPhoneme(0, 2, 50)
